...
router.post("/addClient", async (req, res) => {
    const { name, firstname, email, birthday, phone, password, createdAt } = req.body;
    const securedPassword = await bcrypt.hash(password, 10);
    const sql = "insert into clients (name, firstname, email, birthday,  phone, password, createdAt) values (?,?,?,?,?,?, NOW());";
    const checkSql = "SELECT * FROM clients WHERE email=? OR phone=?;";
    bdd.query(checkSql, [email, phone], (error, result) => {
        if (error) throw error;
        if (result.length > 0) {
            res.status(401).send("Cet email ou ce numéro de téléphone est déjà utilisé. Veuillez en choisir un autre.");
        } else {
            bdd.query(sql, [name, firstname, email, birthday, phone, securedPassword, createdAt], (error, result) => {
                    if (error) throw error;
...
router.patch('/updateClient', auth.authentification, (req, res) => {
    ...
    const updateClient = "UPDATE clients SET name = ?, firstname = ?, email = ?, birthday = ?, address = ?, phone = ? , password= ? WHERE email = ?;";
    bdd.query(updateClient, [name, firstname, email, birthday, address, phone, cryptPassword, req.clientEmail], (error, result) => {
        if (error) throw error;
        res.json(result);
    });
});
    ...
    bdd.query(sql, [email], async (error, results) => {
        ...
        if (results.length > 0) {
            ...
            bcrypt.compare(password, client.password, (error, resultat) => {
                if (error) throw error;
                if (resultat) {
                    const token = jwt.sign({ email: client.email, role: client.role }, 'secretkey', { expiresIn: '2h' });
                    res.json({ token });
                } else {
